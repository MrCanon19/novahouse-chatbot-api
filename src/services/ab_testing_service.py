...
class ABTestingService:
    ...
    @staticmethod
    def assign_variant(experiment_id: int, session_id: str, user_id: str = 'anonymous') -> Optional[Dict[str, Any]]:
        """
        ...
        if existing_participant:
            ...
            variant = next((v for v in variants if v['id'] == existing_participant.variant_id), None)
            return variant
    ...
    def _declare_winner(experiment_id: int) -> Optional[Dict[str, Any]]:
        ...
        if any(r.total_participants < experiment.min_sample_size for r in results):
            return {'message': f'Need at least {experiment.min_sample_size} participants per variant'}
