...
class AdvancedSearchService:
    ...
    def index_knowledge_base(self):
        ...
        try:
            ...
            print(
                f"✅ Indexed {len(FAQ)} FAQs, {len(PORTFOLIO)} projects, {len(CLIENT_REVIEWS)} reviews, {len(BLOG_ARTICLES)} articles"
            )
    ...
    def get_stats(self) -> dict:
        ...
        with self.ix.searcher() as searcher:
            return {
                ...
                "index_size_bytes": (
                    os.path.getsize(os.path.join(INDEX_DIR, "_MAIN_1.toc"))
                    if os.path.exists(os.path.join(INDEX_DIR, "_MAIN_1.toc"))
                    else 0
                ),
            }
