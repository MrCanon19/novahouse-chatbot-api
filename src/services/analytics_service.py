...
from src.models.analytics import ChatAnalytics, UserEngagement
...
class AdvancedAnalytics:
    ...
    def analyze_sentiment(message: str) -> Dict[str, Any]:
        ...
        positive_words = [
            'super', 'świetnie', 'doskonale', 'fantastycznie', 'rewelacja',
            'pięknie', 'profesjonalnie', 'polecam', 'zadowolony', 'zachwycony',
            'dziękuję', 'dzięki', 'wow', 'idealnie', 'perfekcyjnie',
            'tak', 'dobry', 'świetny', 'excellent', 'great', 'good'
        ]
        
        # Negative keywords (Polish)
        negative_words = [
            'źle', 'kiepsko', 'nie', 'słabo', 'okropnie', 'tragedia',
            'beznadziejnie', 'rozczarowany', 'niezadowolony', 'problem',
            'błąd', 'awaria', 'pomyłka', 'zły', 'bad', 'poor', 'terrible'
        ]
    ...
    def get_conversion_funnel(start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        total_sessions = db.session.query(
            func.count(func.distinct(ChatAnalytics.session_id))
        ).filter(
            ChatAnalytics.timestamp >= start_date,
            ChatAnalytics.timestamp <= end_date
        ).scalar() or 0
        ...
        interested_users = db.session.query(
            func.count(func.distinct(ChatAnalytics.session_id))
        ).filter(
            ChatAnalytics.timestamp >= start_date,
            ChatAnalytics.timestamp <= end_date,
        ...
        converted_users = db.session.query(
        ...
        stages = [
            {'name': 'Initial Contact', 'count': total_sessions, 'percentage': 100.0},
            {
                ...
                'count': engaged_users,
                'percentage': round((engaged_users / total_sessions * 100) if total_sessions > 0 else 0, 2)
            },
            {
                'name': 'Interest (pricing/services)',
                'count': interested_users,
                'percentage': round((interested_users / total_sessions * 100) if total_sessions > 0 else 0, 2)
            },
            {
                'name': 'Intent (contact/booking)',
                'count': intent_users,
                'percentage': round((intent_users / total_sessions * 100) if total_sessions > 0 else 0, 2)
            },
            {
                'name': 'Conversion (lead)',
                'count': converted_users,
                'percentage': round((converted_users / total_sessions * 100) if total_sessions > 0 else 0, 2)
            }
        ]
    ...
    def get_cohort_analysis(
        ...
        cohort_query = db.session.query(
            ...
            ChatAnalytics.timestamp >= now - (period_delta * num_cohorts)
        ).group_by(ChatAnalytics.user_id).subquery()
        ...
        interactions = db.session.query(
            ...
            ChatAnalytics,
            ChatAnalytics.user_id == cohort_query.c.user_id
        ).all()
        ...
        for user_id, cohort_date, interaction_date in interactions:
            ...
            period_num = int((interaction_date - cohort_date).total_seconds() / period_delta.total_seconds())
        ...
        for cohort_date in sorted(cohort_data.keys(), reverse=True):
            ...
            for period in range(max_period + 1):
                ...
                retention_pct = round((returning_users / total_users * 100) if total_users > 0 else 0, 2)
    ...
    def get_user_journey_insights(session_id: str) -> Dict[str, Any]:
        ...
        messages = ChatAnalytics.query.filter_by(session_id=session_id).order_by(ChatAnalytics.timestamp).all()
        ...
        if conversion_score >= 70:
            recommendations.append("High conversion probability - follow up with personalized offer")
