...
def get_metrics_summary():
    ...
    try:
        ...
        # Total conversations
        total_conversations = db.session.query(ChatSession).filter(
        ...
        # Total leads
        total_leads = db.session.query(Lead).filter(
...
def get_top_packages():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'data': [
                {'package': row.interested_package, 'count': row.count}
...
def get_active_sessions():
    ...
    try:
        ...
        active_count = db.session.query(ChatSession).filter(
...
def get_response_times():
    ...
    try:
        ...
        bot_messages = db.session.query(Message).filter(
            Message.created_at >= start_date,
            Message.sender == 'bot'
        ).count()
