...
class Experiment(db.Model):
    ...
    )  # [{"id": "A", "name": "Control", "content": "..."}, {"id": "B", ...}]

    # Traffic allocation
    traffic_allocation = db.Column(
        db.Float, default=1.0
    )  # 0.0-1.0 (percentage of users in experiment)
    ...
    def to_dict(self):
        return {
            ...
            "winner_declared_at": (
                self.winner_declared_at.isoformat() if self.winner_declared_at else None
            ),
        }
    ...
    def to_dict(self):
        return {
            ...
            "avg_session_duration": (
                round(self.avg_session_duration, 2) if self.avg_session_duration else 0
            ),
