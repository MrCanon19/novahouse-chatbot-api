...
def import_training_data():
    ...
    with app.app_context():
        ...
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))  # /home/ubuntu/novahouse_project
        ...
        for intent_name, intent_info in all_intents_data.items():
            ...
            print(f"✅ Dodano intencję: {intent_name} ({len(intent_info['training_phrases'])} fraz)")
        ...
        for entity_name, entity_values in all_entities_data.items():
            ...
            entity = Entity(
                name=entity_name,
                values=json.dumps(entity_values, ensure_ascii=False)
            )
        ...
        for intent_name, intent_info in additional_intents.items():
            ...
            if not existing_intent:
                intent = Intent(
                    name=intent_name,
                    training_phrases=json.dumps(intent_info['training_phrases'], ensure_ascii=False),
                    response_templates=json.dumps(intent_info['response_templates'], ensure_ascii=False)
                )
                db.session.add(intent)
