...
def _ensure_id_default(connection):
    """Guarantee the id column has a sequence/identity default for inserts."""

    # Identity or serial both expose a non-null column_default
    default_query = text(
        """
    ...
    print("🔧 Setting identity default on chat_conversations.id")
    connection.execute(
        text(
            "ALTER TABLE chat_conversations ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"
        )
    )
...
def ensure_id_column():
    ...
    with app.app_context():
        ...
        with db.engine.begin() as connection:
            ...
            # 2. Backfill NULL ids using the attached identity/sequence
            connection.execute(
                text(
                    """
