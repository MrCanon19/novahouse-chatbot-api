...
def benchmark_endpoint(url: str, method: str = "GET", data: Dict = None, 
...
def print_benchmark_result(name: str, result: Dict):
    ...
    print(f"  Mean: {mean:.0f}ms | Median: {result['median']:.0f}ms | "
          f"Min: {result['min']:.0f}ms | Max: {result['max']:.0f}ms")
    print(f"  Std Dev: {result['stdev']:.0f}ms | Error Rate: {result['error_rate']:.0f}% | "
